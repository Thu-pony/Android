...
public class FileUtils {
    ...
    public  static  void writeBytes(byte[] array, Context context) {
        ...
        try {
            ...
            writer = new FileOutputStream(context.getFilesDir() + "/codec.h264", true);
            Log.d(TAG, "PATH = " + context.getFilesDir());
            writer.write(array);
            writer.write('\n');
    ...
    public  static String writeContent(byte[] array, Context context) {
        ...
        try {
            // 打开一个写文件器，构造函数中的第二个参数true表示以追加形式写文件
            writer = new FileWriter(context.getFilesDir() + "/codecH264.txt", true);
            Log.d(TAG, "PATH = " + context.getFilesDir());
