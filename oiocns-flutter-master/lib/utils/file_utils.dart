...
class FileUtils {
  ...
  static bool isMarkDown(String extension) {
    extension = extension.toLowerCase();
    return extension.contains("md");
  }

  static bool isDownloadFile(String extension) {
    if (Platform.isIOS) {
      return !isWord(extension) &&
          !isPdf(extension) &&
          !isVideo(extension) &&
          !isMarkDown(extension);
    } else {
      return isWord(extension) ||
          (!isPdf(extension) && !isVideo(extension) && !isMarkDown(extension));
    }
  }
}
