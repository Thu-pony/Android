...
import 'package:orginone/components/common/extension/ex_datetime.dart';
...
import 'package:orginone/dart/base/storages/models/user_model.dart';
...
// 存储集线器
  ...
  String get statisticsInfo {
    return _storeHub.getStatisticsInfo();
  }
...
//   }
  ...
  Future<bool> tokenAuth() async {
    ...
    if (_storeHub.accessToken.isNotEmpty) {
      ...
      if (result.success) {
        ...
        Storage.setJson(Constants.sessionUser, result.data);
        return true;
      } else {
        LogUtil.e('连接到内核失败!');
        LogUtil.e(
            "请求时间:${DateTime.now().toDateString(format: 'yyyy-MM-dd hh:mm:ss.S')}");
        LogUtil.e('token:${_storeHub.accessToken}');
        LogUtil.w(jsonEncode(result));
      }
    }
    return false;
  }
  ...
  Future<ResultType<dynamic>> login(String userName, String password) async {
    ...
    if (res.success) {
      ...
      bool tokenAuthRes = await tokenAuth();
      if (tokenAuthRes) {
        Storage.setBool(Constants.loginStatus, true);
      } else {
        res.code = 401;
        res.success = false;
        res.msg = "token认证失败";
  ...
  Future<LoadResult<T>> collectionLoad<T>(
    ...
    ResultType res = await dataProxy(params);
  ...
  Future<ResultType<bool>> dataNotify(DataNotityType req) async {
    ...
    return ResultType<bool>.fromObj(
        res, res.data == null ? false : res.data as bool);
  }
  ...
  void subscribe(
    ...
    // ToastUtils.showMsg(msg: "添加监听：$flag");
  ...
  /// @param keys 唯一标志
  /// @param operation 操作
  unSubscribe({String? flag, String? key}) {
    // ToastUtils.showMsg(msg: "删除监听：$flag $key");
    if (null != flag) {
      flag = flag.toLowerCase();
      _subMethods[flag] = [];
    } else if (null != key) {
  ...
  _receive(List<dynamic>? params) {
    ...
    _storeHub.toStatistic(ResultType.success(res), "dataNotify");
    switch (res.target) {
      case 'DataNotify':
        {
          ...
          //KernelApi 发布订阅不稳定，可能销毁实际有问题 Team的订阅经常收不到 使用command 替代
          //KernelApi 发布订阅稳定后考虑统一
          // command.emitterFlag(flag, [data.data]);
          var methods = _subMethods[flag];
          if (null != methods) {
            // ToastUtils.showMsg(msg: "监听数量：${methods.length}");
            ...
            try {
              ...
              LogUtil.w(s);
              LogUtil.w(e as Error);
            }
          } else {
            // ToastUtils.showMsg(msg: "没有监听：$flag");
      ...
      default:
        {
          ...
          if (methods != null) {
            try {
              for (var m in methods) {
                // LogUtil.d(res.target);
                // LogUtil.d(m.runtimeType);
                m.call(res.data);
