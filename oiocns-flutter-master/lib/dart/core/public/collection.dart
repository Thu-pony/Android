...
class XCollection<T extends Xbase> {
  ...
  Future<List<T>> all(
    ...
    if (!_loaded || reload) {
      ...
      var res = await loadResult(params, fromJson);
      if (res.success) {
        if (null != res.data && res.data!.isNotEmpty) {
          ...
          if (this._cache.length < res.totalCount && res.data!.length == 100) {
            await this.all(
                reload: true,
                skip: this._cache.length,
                storeId: storeId,
                fromJson: fromJson);
            // await this
            //     .all1(reload: true, skip: this._cache.length, storeId: storeId);
          }
        }
        this._loaded = true;
      }
  ...
  void unsubscribe({String? id, String? key}) {
    kernel.unSubscribe(flag: subMethodName(id: id), key: key);
  }
}
