...
class Session extends Entity<XEntity> implements ISession {
  ...
  void readMessages(List<IMessage> messages, [Message? msg]) {
    ...
    if (readCount > 0) {
      chatdata.value.noReadCount -= min(readCount, chatdata.value.noReadCount);
      cacheChatData(true);
  ...
  Future<bool> cacheChatData([bool? notify = false]) async {
    ...
    // ToastUtils.showMsg(
    //     msg:
    //         "更新会话未读数：${relationCtrl.appStartController.appStartStatus.name} ${chatdata.value.noReadCount}");
  ...
  void newMessage(ChatMessageType data, IMessage msg) {
    if (_isNoValidNewMessage(msg)) return;
