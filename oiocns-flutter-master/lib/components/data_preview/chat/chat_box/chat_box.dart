...
import 'package:orginone/components/data_preview/chat/chat_box/at/at_person_dialog.dart';
import 'package:orginone/components/data_preview/chat/chat_box/at/at_textfield.dart';
import 'package:orginone/components/data_preview/chat/chat_box/message_forward.dart';
import 'package:orginone/components/data_preview/chat/chat_box/rich_text_input_formatter.dart';
...
class ChatBox extends StatelessWidget with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((duration) {
      var bottom =
          MediaQueryData.fromView(View.of(Get.context!)).padding.bottom;
  ...
  Widget _emojiBtn(BuildContext context) {
    return GestureDetector(
      ...
      child: _rightIcon(XImage.iconEmoji),
    );
  }
  ...
  Widget _moreBtn(BuildContext context) {
    return GestureDetector(
      ...
      child: _rightIcon(XImage.iconAddAction),
    );
  }
  ...
  Widget _voiceBtn(BuildContext context) {
    return GestureDetector(
      ...
      child: _leftIcon(XImage.iconVoice),
    );
  }
...
class ChatBoxController with WidgetsBindingObserver {
  ...
  ChatBoxController() {
    ...
    atKey = GlobalKey();
    if (null != Get.context) {
      PermissionUtil.showPermissionDialog(Get.context!, Permission.microphone,
          isShowTip: false);
    } else {
  ...
  eventFire(BuildContext context, InputEvent inputEvent, ISession chat) async {
    switch (inputEvent) {
      ...
      case InputEvent.clickSendBtn:
        ...
        var success = await chat.sendMessage(MessageType.text, message,
  ...
  void imagePicked(XFile pickedImage, ISession chat) async {
    ...
    chat.sendMessage(
        ...
        jsonEncode(FileItemShare(
                extension: '.$ext',
                shareLink: pickedImage.path,
                name: pickedImage.name,
                size: 0)
            .toJson()),
        [], uploadTaskCallback: (IMessage msg) async {
      ...
      if (item != null) {
        ...
        msg.msgSource = jsonEncode(item.shareInfo().toJson());
        // chat.sendMessage(
  ...
  Future<void> filePicked(PlatformFile file, ISession chat) async {
    ...
    chat.sendMessage(
      ...
      if (item != null) {
        // chat.messages.remove(msg);
        msg.msgType = MessageType.file.label;
        msg.msgSource = jsonEncode(item.shareInfo().toJson());
        // chat.sendMessage(MessageType.file,
        //     jsonEncode(item.shareInfo().toJson()), msg.mentions);
      }
    });
  }
  ...
  execute(
    ...
    switch (moreFunction) {
      ...
      case MoreFunction.file:
        ...
        break;
      case MoreFunction.card:
        showModalBottomSheet(
            context: context,
            builder: (context) {
              return MessageForward(
                msgBody: jsonEncode(chat.metadata),
