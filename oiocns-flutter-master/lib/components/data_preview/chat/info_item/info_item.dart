...
import 'package:orginone/components/common/getx/event_bus_helper.dart';
...
import 'package:orginone/components/data_preview/chat/info_item/parse_msg/business_card.dart';
import 'package:orginone/components/data_preview/chat/info_item/parse_msg/file_detail.dart';
import 'package:orginone/components/data_preview/chat/info_item/parse_msg/group_dynamics_detail.dart';
import 'package:orginone/components/data_preview/chat/info_item/parse_msg/image_detail.dart';
import 'package:orginone/components/data_preview/chat/info_item/parse_msg/text_detail.dart';
import 'package:orginone/components/data_preview/chat/info_item/parse_msg/uploading_detail.dart';
import 'package:orginone/components/data_preview/chat/info_item/parse_msg/voice_detail.dart';
...
class _DetailItemWidgetState extends State<DetailItemWidget> {
  ...
  IndexController get controller => Get.find<IndexController>();
  late ISession chat;
  late IMessage msg;
  late String key;
  ...
  Widget _messageDetail(BuildContext context) {
    ...
    if (msg.msgType == MessageType.recall.label) {
      ...
      children.add(Row(
        ...
        children: [
          if (msg.sendState == ExecuteStatus.starting && isSelf)
            FutureBuilder(
          ...
          if (msg.sendState == ExecuteStatus.failed && isSelf)
            InkWell(
              onTap: () {
                chat.resendMessage(msg);
              },
              radius: 0,
  ...
  Widget _chatBody(BuildContext context, TextDirection textDirection) {
    ...
    } else if (msg.msgType == MessageType.activity.label) {
      body = GroupDynamicsDetail(isSelf: isSelf, message: msg, chat: chat);
    } else if (msg.msgType == MessageType.card.label) {
      body = BusinessCardDetail(isSelf: isSelf, message: msg, chat: chat);
    } else {
...
//   @override
//   Widget build(BuildContext context) {
//     msg.subscribe((key, p1) {
//       print("change status=11111111");
//     });
