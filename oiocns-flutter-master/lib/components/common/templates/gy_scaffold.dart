...
import 'package:orginone/components/common/icons/user_bar.dart';
...
class _GyScaffoldState extends State<GyScaffold> {
  ...
  void didUpdateWidget(covariant GyScaffold oldWidget) {
    ...
    if (oldWidget.backgroundColor != widget.backgroundColor) {
      backgroundColor = widget.backgroundColor ?? XColors.backgroundColor;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: scaffoldKey,
      appBar: _buildTitle(isHomePage, scaffoldKey),
      ...
      drawerEnableOpenDragGesture: isHomePage ? true : false,
    );
  }
  ...
  /// 构建二级页面头部
  PreferredSizeWidget _buildTitle(
  ...
  buildDrawer() {
    return Container(
        ...
        child: Column(
          children: [
            UserBar().settingMenuHeaderWidget(),
            Padding(
            ...
            ...SettingEnum.values
                ...
                .map(
                  (item) => UserBar().settingMenuItemWidget(item),
                )
                .toList(),
            const Expanded(child: SizedBox()),
            UserBar().settingMenuItemWidget(SettingEnum.exitLogin,
