import 'package:orginone/components/common/getx/base_list_controller.dart';
...
class WorkBenchController extends BaseListController<WorkBenchState> {
  ...
  void onInit() async {
    ...
    //   if ((relationCtrl.user?.companys.length ?? 0) > 0) {
    //     LogUtil.d(">>>>>>>单位数量${relationCtrl.user?.companys.length}");
    //     state.colleagueNum.value = relationCtrl.user?.companys
    //             .map((i) {
    //               if (i.members.isNotEmpty) {
    //                 return i.members.map((e) => e.id);
    //               }
    //               return ;
    //             })
    //             .reduce(
