...
import 'package:orginone/components/common/button/text_arrow.dart';
import 'package:orginone/components/common/dialogs/common_widget.dart';
import 'package:orginone/components/common/extension/ex_string.dart';
...
class _WorkBenchPageState extends State<WorkBenchPage> {
  ...
  void initState() {
    ...
    // relationCtrl.work.todos
    loadApp();
  }

  loadApp() async {
    listApp.value = (await relationCtrl.loadCommons())
        .where((element) => element.typeName == DirectoryType.app.label)
        .toList();

    // applications.value = (await relationCtrl.loadApplications())
  ...
  renderCmdBtn(IFileInfo<XEntity> file) {
    return GestureDetector(
      onTap: () {
        ...
        //   RoutePages.jumpStore(parentData: file, listDatas: []);
        // }
      },
      child: SizedBox(
        width: 120.w,
  ...
  Widget _renderRecentApp() {
    ...
    return Obx(() => modelWindow("最近应用",
        contentWidget: GridView.count(
            ...
            children: listApp
                .sublist(0, min(8, listApp.length))
        ...
        contentWidget: Container(
            ...
            child: Row(
              ...
              children: [
                _buildWorkItemV1(
                    "未读",
                    relationCtrl.provider.chatProvider?.noReadChatCount
                            .toString() ??
                        "0",
                    XImage.homeChat,
                ...
                _buildWorkItemV1("待办", todoCount.value, XImage.homeWork,
  ...
  //                           () {},
  //                           -1,
  //                           '共计:${relationCtrl.provider.chatProvider?.chats.length ?? 0}个'),
  //                       renderDataItem(
  //                           '数据集(个)',
  //                           null,
